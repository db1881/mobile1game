...
bu APK üzerinde gerçek hesap seçme ve skor gönderme cihaz testidir. Ayrıca toplam yıldız liderliği
ile bağlı kümeleri gezen lazer/patlama güncellemesini birleştiren güncel imzalı AAB
23 Temmuz 2026'da `0.1.3-internal-4` (version code `4`) adıyla Dahili test kanalında yayınlandı;
...
2026-07-22'de liderlik metriği toplam yıldıza çevrildi. `GooglePlayGamesService` artık
`SaveSystem.GetTotalStars()` değerini gönderiyor, iki Google sayfasıyla ilk 50 oyuncuyu alıyor ve
son başarılı listeyi çevrimdışı görüntülemek için `PlayerPrefs` içinde saklıyor. Panel kaydırılabilir
`YILDIZ LİGİ` tasarımına geçti; ana menüde Google rozeti ve kupa ikonu eklendi. Yeni bir Play Games
leaderboard `Total Stars` (`CgkI__bSw58ZEAIQAw`) oluşturuldu, kimliği
